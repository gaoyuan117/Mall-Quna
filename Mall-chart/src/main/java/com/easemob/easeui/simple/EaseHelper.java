...
import com.easemob.easeui.simple.InviteMessage.InviteMesageStatus;
...
public class EaseHelper
{
	...
	private EMOptions initChatOptions()
	{
		...
		options.setDeleteMessagesAsExitGroup(getModel()
				.isDeleteMessagesAsExitGroup());
		options.setAutoAcceptGroupInvitation(getModel()
	...
	protected void setEaseUIProviders()
	{
		...
		easeUI.setEmojiconInfoProvider(new EaseEmojiconInfoProvider()
		{
			...
			public EaseEmojicon getEmojiconInfo(String emojiconIdentityCode)
			{
				...
				for (EaseEmojicon emojicon : data)
				{
	...
	class MyGroupChangeListener implements EMGroupChangeListener
	{
				...
				String reason)
		{
			...
			EMGroup _group = null;
			for (EMGroup group : EMClient.getInstance().groupManager()
					.getAllGroups())
			{
			...
			Log.d(TAG,
					invitee + "同意加入群聊：" + _group == null ? groupId : _group
							.getGroupName());
				...
				String reason)
		{
			...
			boolean hasGroup = false;
			EMGroup group = null;
			for (EMGroup _group : EMClient.getInstance().groupManager()
					.getAllGroups())
			{
			...
			msg.setGroupInviter(invitee);
			Log.d(TAG,
					invitee + "拒绝加入群聊：" + group == null ? groupId : group
							.getGroupName());
	...
	public class MyContactListener implements EMContactListener
	{
		...
		public void onContactDeleted(String username)
		{
			...
			Map<String, EaseUser> localUsers =
					EaseHelper.getInstance().getContactList();
			...
			public void onCmdMessageReceived(List<EMMessage> messages)
			{
				for (EMMessage message : messages)
				{
					...
					final String CMD_TOAST_BROADCAST =
							"hyphenate.demo.cmd.toast";
					IntentFilter cmdFilter =
							new IntentFilter(CMD_TOAST_BROADCAST);
		...
		new Thread()
		{
			...
			public void run()
			{
				...
				try
				{
					...
					List<EaseUser> users =
							new ArrayList<EaseUser>(userlist.values());
	...
	public void login(String currentUsername, String currentPassword)
	{
